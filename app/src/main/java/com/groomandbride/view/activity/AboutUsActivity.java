...
public class AboutUsActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTitleBar.setText("About Us");
        setSupportActionBar(toolbar);

        backBtn.setOnClickListener(view -> finish());

        //getAboutUs();
        aboutTv.setText("Groom & Bride is the only wedding halls platform representer in Egypt.");
    }

    private void getAboutUs() {
        showProgress();
        ApiInterface apiService = RetrofitApiClient.getClient().create(ApiInterface.class);
        Call<PrivacyPolicyTermsModel> resPrivacy = apiService.getPrivacyPolicy("about");
        resPrivacy.enqueue(new Callback<PrivacyPolicyTermsModel>() {
            ...
            public void onResponse(Call<PrivacyPolicyTermsModel> call, Response<PrivacyPolicyTermsModel> response) {
                hideProgress();
                aboutTv.setText(Html.fromHtml(response.body().getData().getText()));
            }
